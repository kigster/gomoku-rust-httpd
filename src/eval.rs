...
static mut THREAT_COST: [i32; 20] = [0; 20];
...
pub fn populate_threat_matrix() {
    INIT.call_once(|| unsafe {
        ...
        THREAT_COST[THREAT_FOUR_BROKEN] = 8000;
        THREAT_COST[THREAT_THREE] = 1000;
        THREAT_COST[THREAT_THREE_BROKEN] = 200;
        THREAT_COST[THREAT_TWO] = 50;
        THREAT_COST[THREAT_NEAR_ENEMY] = 10;
        THREAT_COST[THREAT_THREE_AND_FOUR] = 45000;
        THREAT_COST[THREAT_THREE_AND_THREE] = 40000;
        THREAT_COST[THREAT_THREE_AND_THREE_BROKEN] = 5000;
    });
}

#[inline]
fn threat_cost(idx: usize) -> i32 {
    unsafe { THREAT_COST[idx] }
}
...
pub fn calc_score_at(board: &Board, player: i32, x: i32, y: i32) -> i32 {
    ...
    let row_size = (SEARCH_RADIUS * 2 + 1) as usize;
    let center = SEARCH_RADIUS as usize;
    let mut threats = [0usize; NUM_DIRECTIONS];

    // Horizontal
    let mut row = vec![OUT_OF_BOUNDS; row_size];
    row[center] = player;
...
pub fn evaluate_threat_fast(board: &Board, x: i32, y: i32, player: i32) -> i32 {
    ...
    if num_open_twos >= 2 {
        max_threat = max_threat.max(2000);
    }
    if num_open_twos >= 1 && num_open_threes >= 1 {
        max_threat = max_threat.max(3000);
    }
